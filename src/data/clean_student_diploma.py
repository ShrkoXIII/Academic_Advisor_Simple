...
from src.data.cleaning_utils import clean_column_names, clean_id_columns, to_float
from src.paths import (
    ACADEMIC_INFO_PATH,
    CLEAN_STUDENT_DIPLOMA_PATH_V2,
    STUDENT_COURSE_DIPLOMA_PATH_V2,
...
def main():
    ...
    CLEAN_STUDENT_DIPLOMA_PATH_V2.parent.mkdir(parents=True, exist_ok=True)
    student_diploma.to_parquet(CLEAN_STUDENT_DIPLOMA_PATH_V2, index=False)

    student_course = pd.read_parquet(STUDENT_COURSE_ENRICHED_PATH_V2)
    merged = merge_student_course_with_diploma(
        student_course,
        student_diploma,
    )

    STUDENT_COURSE_DIPLOMA_PATH_V2.parent.mkdir(parents=True, exist_ok=True)
    merged.to_parquet(STUDENT_COURSE_DIPLOMA_PATH_V2, index=False)
    ...
    print("Saved clean diploma:", CLEAN_STUDENT_DIPLOMA_PATH_V2)
    print("Merged rows:", len(merged))
    print("Matched rows:", matched_rows)
    print("Unmatched rows:", len(merged) - matched_rows)
    print("Saved merged data:", STUDENT_COURSE_DIPLOMA_PATH_V2)
