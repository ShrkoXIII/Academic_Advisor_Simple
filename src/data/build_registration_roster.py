...
from src.data.build_temporal_split import TEST_PARTS, TRAIN_PARTS
...
def build_registration_roster(raw_courses, student_status, degree_courses, outliers):
    ...
    roster = roster.merge(plan, on=["degree_id", "course_id"], how="left", validate="many_to_one")
...
def main():
    roster = build_registration_roster(
        ...
        pd.read_parquet(CLEAN_STUDENT_STATUS_PATH_V2),
        pd.read_parquet(CLEAN_DEGREE_COURSE_PATH_V2),
        pd.read_parquet(OUTLIER_STUDENTS_AUDIT_PATH_V2),
    )
    ...
    CLEAN_REGISTRATION_ROSTER_PATH_V2.parent.mkdir(parents=True, exist_ok=True)
    TEMPORAL_TRAIN_ROSTER_PATH_V2.parent.mkdir(parents=True, exist_ok=True)
    roster.to_parquet(CLEAN_REGISTRATION_ROSTER_PATH_V2, index=False)
    temporal_train_roster.to_parquet(TEMPORAL_TRAIN_ROSTER_PATH_V2, index=False)
    temporal_test_roster.to_parquet(TEMPORAL_TEST_ROSTER_PATH_V2, index=False)
    ...
    print("Saved:", CLEAN_REGISTRATION_ROSTER_PATH_V2)
    print("Saved:", TEMPORAL_TRAIN_ROSTER_PATH_V2)
    print("Saved:", TEMPORAL_TEST_ROSTER_PATH_V2)
