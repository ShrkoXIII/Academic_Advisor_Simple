...
def main():
    df = pd.read_parquet(STUDENT_COURSE_WITHOUT_OUTLIERS_PATH_V2)
    temporal_train, temporal_test, incomplete = build_temporal_split(df)

    TEMPORAL_TRAIN_PATH_V2.parent.mkdir(parents=True, exist_ok=True)
    temporal_train.to_parquet(TEMPORAL_TRAIN_PATH_V2, index=False)
    temporal_test.to_parquet(TEMPORAL_TEST_PATH_V2, index=False)
    ...
    print("Saved temporal train:", TEMPORAL_TRAIN_PATH_V2)
    print("Saved temporal test:", TEMPORAL_TEST_PATH_V2)
