...
def main():
    df = pd.read_parquet(STUDENT_COURSE_DIPLOMA_PATH_V2)
    audit = build_outlier_audit(df)
    clean_df = remove_outlier_students(df, audit)

    STUDENT_COURSE_WITHOUT_OUTLIERS_PATH_V2.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
    clean_df.to_parquet(STUDENT_COURSE_WITHOUT_OUTLIERS_PATH_V2, index=False)
    audit.to_parquet(OUTLIER_STUDENTS_AUDIT_PATH_V2, index=False)
    ...
    print("Saved clean data:", STUDENT_COURSE_WITHOUT_OUTLIERS_PATH_V2)
    print("Saved outlier audit:", OUTLIER_STUDENTS_AUDIT_PATH_V2)
