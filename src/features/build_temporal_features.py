...
def main():
    train, test, state = build_feature_tables(
        pd.read_parquet(TEMPORAL_TRAIN_PATH_V2),
        pd.read_parquet(TEMPORAL_TEST_PATH_V2),
        pd.read_parquet(TEMPORAL_TRAIN_ROSTER_PATH_V2),
        pd.read_parquet(TEMPORAL_TEST_ROSTER_PATH_V2),
        pd.read_parquet(CLEAN_STUDENT_STATUS_PATH_V2),
    )

    TEMPORAL_TRAIN_FEATURES_PATH_V2.parent.mkdir(parents=True, exist_ok=True)
    train.to_parquet(TEMPORAL_TRAIN_FEATURES_PATH_V2, index=False)
    test.to_parquet(TEMPORAL_TEST_FEATURES_PATH_V2, index=False)
    save_course_history_state(state, COURSE_HISTORY_STATE_PATH_V2)
    ...
    print("Saved:", TEMPORAL_TRAIN_FEATURES_PATH_V2)
    print("Saved:", TEMPORAL_TEST_FEATURES_PATH_V2)
    print("Saved:", COURSE_HISTORY_STATE_PATH_V2)
