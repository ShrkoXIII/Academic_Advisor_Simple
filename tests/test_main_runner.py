...
def test_list_displays_standard_and_optional_work_without_execution(monkeypatch, capsys):
    ...
    for item in ["data", "features", "modeling", "evaluation", "experiments",
                 ...
                 "evaluate_xml_recommendations", "V2", "V1"]:
        assert item in output


def test_all_dry_run_shows_commands_and_version_boundary_without_execution(monkeypatch, capsys):
    def forbidden(*args, **kwargs):
    ...
    assert "VERSION BOUNDARY" in output
    assert "V2" in output and "V1" in output
...
def test_all_excludes_optional_work_and_delegates_request_arguments(monkeypatch, capsys):
    ...
    assert capsys.readouterr().out.count("VERSION BOUNDARY") == 2
