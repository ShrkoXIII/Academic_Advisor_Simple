...
**مسار توصيات Expected Points المحلي جاهز.** يستقبل قائمة مواد معتمدة،
ويقيّم كل التركيبات المطابقة للساعات على دفعات ويحفظ جميع الخطط الأعلى من
معدل الطالب. دليل الاستخدام: [LOCAL_RECOMMENDATION.md](LOCAL_RECOMMENDATION.md).
الخطوة التالية هي اختبار قائمة يؤمّنها المستخدم ثم تقييم ربط API.
...
- [x] تعداد كل الخطط المطابقة للساعات المحددة أو الواقعة ضمن المجال شاملاً طرفيه، بعدد مواد متغير.
- [x] إعادة حساب خصائص الحمل لكل خطة.
- [x] توقع النقاط مباشرة واحتمال الرسوب لكل مادة ضمن سياق الخطة.
- [x] الاحتفاظ بكل الخطط المطابقة، مع عرض ما إذا كان التوقع التراكمي يتحسن.
- [x] ترتيب الخطط حسب المعدل التراكمي المتوقع، ثم الساعات المتوقع رسوبها، ثم معدل الخطة المتوقع، ثم معرّف ثابت.
- [x] إخراج شرح حمل الخطة وتوقعات كل مادة.
- [x] تنفيذ smoke test على طالب حقيقي من اختبار 2025.

الكود: `src/recommendation/engine.py`، مع التوليد في `plan_generation.py` والترتيب في `plan_scoring.py`.
