...
المشروع يدعم الآن تشغيل توصيات الخطط محلياً من قائمة مواد معتمدة؛
دليل التشغيل والمدخلات والاختبارات: [LOCAL_RECOMMENDATION.md](LOCAL_RECOMMENDATION.md).
...
تنظيف ودمج
  ...
  → توليد كل التركيبات المطابقة للساعات وتقييمها محلياً
  → [الخطوة التالية] تجربة قائمة معتمدة ثم ربط API
```

الوضع الحالي للمودلات:

- **رسمي حاليًا:** `GradeRegressor` و`FailRiskClassifier`.
- **مسار التوصية المحلي:** Expected Points مباشرة مع تاريخ الاختصاص؛ يقرأ
  النسخة المختارة من metadata، وحالياً `degree_history_points_temporal`.
- **لاحقاً:** تحقق على قوائم مؤمّنة للطلاب وربط API؛ تحسن الخطط البديلة المتوقع
  ليس نتيجة فعلية مقاسة.
